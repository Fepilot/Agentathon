# ✅ Agentathon Preparation Checklist

Use this checklist to ensure you're fully prepared for your Agentathon event.

---

## 📅 6 Weeks Before

### Customer/Team Engagement
- [ ] Identify target customer or internal team
- [ ] Schedule initial discovery call
- [ ] Understand business context and objectives
- [ ] Confirm executive sponsorship

### Logistics
- [ ] Determine event format (in-person / virtual / hybrid)
- [ ] Identify potential dates
- [ ] Secure facilitator(s)

---

## 📅 4 Weeks Before

### Environment Setup
- [ ] **Create CDX Demo Tenant** - [See CDX Setup Guide](cdx-demo-tenant-setup.md) | [Download PDF](CDX-Demo-Tenant-Setup.pdf)
- [ ] Verify Copilot Studio licenses for all participants
- [ ] Set up development environments
- [ ] Test connectivity and access
- [ ] Configure any Azure resources needed

### Communications
- [ ] Send save-the-date to participants
- [ ] Share prerequisites checklist
- [ ] Set up Teams channel for the event

---

## 📅 2 Weeks Before

### Content Preparation
- [ ] Customize presentation materials
- [ ] Prepare hands-on exercises
- [ ] Define use case scenarios
- [ ] Create agent templates (if applicable)

### Participant Readiness
- [ ] Distribute pre-reading materials
- [ ] Confirm participant list
- [ ] Address any technical issues

---

## 📅 1 Week Before

### Final Preparations
- [ ] Send final agenda and logistics
- [ ] Confirm catering (if in-person)
- [ ] Test all technical equipment
- [ ] Prepare backup plans for common issues

### Materials Ready
- [ ] Presentation deck finalized
- [ ] Exercise instructions printed/shared
- [ ] Demo environments tested
- [ ] Feedback forms prepared

---

## 📅 Day Before

- [ ] Final environment check
- [ ] Confirm all participants received materials
- [ ] Set up room (if in-person)
- [ ] Test recording setup (if applicable)

---

## 📅 Event Day

### Morning
- [ ] Arrive early for final setup
- [ ] Test projector/screens
- [ ] Welcome participants
- [ ] Take attendance

### Throughout the Day
- [ ] Facilitate sessions per agenda
- [ ] Capture photos/screenshots
- [ ] Document questions and insights
- [ ] Ensure breaks are taken

### End of Day
- [ ] Collect feedback
- [ ] Capture demo recordings
- [ ] Thank participants
- [ ] Announce next steps

---

## 📅 Post-Event

- [ ] Send thank-you email
- [ ] Share recordings and materials
- [ ] Analyze feedback
- [ ] Document success story
- [ ] Schedule follow-up meeting
- [ ] Update this repository with learnings

---

## 📝 Notes

Add any specific notes for your event here:

```
[Your notes]
```
